...
public class AshCommandExecutor implements CommandExecutor
{
	private AdvancedStoreHouse plugin;
	private static CommandConstructor cc;
	
	public AshCommandExecutor(AdvancedStoreHouse plugin, CommandConstructor cc)
	{
		this.plugin = plugin;
		AshCommandExecutor.cc = cc;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lable, String[] args) 
	{
		...
		{
			AdvancedStoreHouse.log.info("/%cmd% is only for Player!".replace("%cmd%", cc.getName()));
			return false;
		}
				...
				if(args[i].equalsIgnoreCase(ac.getName()))
				{
					if(length >= ac.minArgsConstructor && length <= ac.maxArgsConstructor)
					{
						if(player.hasPermission(ac.getPermission()))
						{
							...
							{
								AdvancedStoreHouse.log.info("ArgumentModule from ArgumentConstructor %ac% not found! ERROR!"
					...
					}/* else if(length > ac.maxArgsConstructor) 
					{
						...
						player.spigot().sendMessage(ChatApi.clickEvent(plugin.getYamlHandler().getL().getString("InputIsWrong"),
								ClickEvent.Action.RUN_COMMAND, AdvancedStoreHouse.infoCommand));
						return false;
					}*/ else
					{
	...
	public void baseCommands(final Player player, int page)
	{
		...
		player.spigot().sendMessage(ChatApi.tctl(plugin.getYamlHandler().getL().getString(
				AdvancedStoreHouse.infoCommandPath+".BaseInfo.Headline")));
